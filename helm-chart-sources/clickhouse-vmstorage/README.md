...
|---|---|---|
|kube_node_status_capacity|kube-state-metrics|{{ .Values.companyPrefix }}_nodegroup_memory_total_mb {{ .Values.companyPrefix }}_nodegroup_cpu_total_cores {{ .Values.companyPrefix }}_nodegroup_filesystem_total_mb {{ .Values.companyPrefix }}_nodegroup_filesystem_used_percent {{ .Values.companyPrefix }}_node_memory_total_mb {{ .Values.companyPrefix }}_node_cpu_total_cores {{ .Values.companyPrefix }}_node_filesystem_total_mb {{ .Values.companyPrefix }}_node_filesystem_used_percent|
|kube_node_labels|kube-state-metrics|{{ .Values.companyPrefix }}_nodegroup_memory_total_mb {{ .Values.companyPrefix }}_nodegroup_memory_used_mb {{ .Values.companyPrefix }}_nodegroup_cpu_total_cores {{ .Values.companyPrefix }}_nodegroup_filesystem_total_mb {{ .Values.companyPrefix }}_nodegroup_filesystem_used_percent|
|container_memory_working_set_bytes|cadvisor|{{ .Values.companyPrefix }}_nodegroup_memory_used_mb {{ .Values.companyPrefix }}_node_memory_used_mb|
|node_cpu_usage_seconds_total|resource|{{ .Values.companyPrefix }}_nodegroup_cpu_used_cores {{ .Values.companyPrefix }}_node_cpu_used_cores|
|container_fs_usage_bytes|cadvisor|{{ .Values.companyPrefix }}_nodegroup_filesystem_used_mb {{ .Values.companyPrefix }}_nodegroup_filesystem_used_percent {{ .Values.companyPrefix }}_node_filesystem_used_mb {{ .Values.companyPrefix }}_node_filesystem_used_percent|
|kube_pod_status_phase|kube-state-metrics|{{ .Values.companyPrefix }}_pod_status_phase|
|kube_pod_container_status_restarts_total|kube-state-metrics|{{ .Values.companyPrefix }}_pod_restarts_total|
|kube_pod_info|kube-state-metrics|{{ .Values.companyPrefix }}_pod_info|
|container_memory_working_set_bytes|cadvisor|{{ .Values.companyPrefix }}_pod_memory_usage_mb|
|container_cpu_usage_seconds_total|cadvisor|{{ .Values.companyPrefix }}_pod_cpu_usage_cores|

## Метрики групп нод
Метрики и примеры графиков можно посмотреть в графане на дашборде {{ .Values.companyPrefix }}-grafana -> ClickhouseTSDB / Nodegroups (по пути https://GRAFANA_ADDR/grafana/d/clickhouse-tsdb-nodegroups)

### {{ .Values.companyPrefix }}_nodegroup_memory_used_mb
#### Описание
  ...
  * on(nodename) group_left(nodegroup)
  max by (nodename, nodegroup) (
    label_replace(
      ...
      "nodegroup", "$1", "label_node_group_{{ .Values.companyPrefix }}_com_name", "(.*)"
    )
  )
)
```
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_nodegroup_memory_used_mb'
...
### {{ .Values.companyPrefix }}_nodegroup_memory_total_mb
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_nodegroup_memory_total_mb'
...
### {{ .Values.companyPrefix }}_nodegroup_cpu_used_cores
#### Описание
...
### {{ .Values.companyPrefix }}_nodegroup_cpu_total_cores
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_nodegroup_cpu_total_cores'
  AND d.timestamp >= toDateTime(1770500419) AND d.timestamp <= toDateTime(1770543619)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodegroup'] IN ('linux')
GROUP BY time, metric
...
### {{ .Values.companyPrefix }}_nodegroup_filesystem_used_mb
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_nodegroup_filesystem_used_mb'
  AND d.timestamp >= toDateTime(1770500419) AND d.timestamp <= toDateTime(1770543619)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodegroup'] IN ('linux')
GROUP BY time, metric
...
### {{ .Values.companyPrefix }}_nodegroup_filesystem_total_mb
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_nodegroup_filesystem_total_mb'
  AND d.timestamp >= toDateTime(1770500419) AND d.timestamp <= toDateTime(1770543619)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodegroup'] IN ('linux')
GROUP BY time, metric
...
### {{ .Values.companyPrefix }}_nodegroup_filesystem_used_percent
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_nodegroup_filesystem_used_percent'
  AND d.timestamp >= toDateTime(1770500419) AND d.timestamp <= toDateTime(1770543619)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodegroup'] IN ('linux')
GROUP BY time, metric
...
## Метрики нод
Метрики и примеры графиков можно посмотреть в графане на дашборде {{ .Values.companyPrefix }}-grafana -> ClickhouseTSDB / Nodes (по пути https://GRAFANA_ADDR/grafana/d/clickhouse-tsdb-nodes)

### {{ .Values.companyPrefix }}_node_memory_used_mb
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_node_memory_used_mb'
...
### {{ .Values.companyPrefix }}_node_memory_total_mb
#### Описание
Объем RAM на ноде
#### Лейблы
- cluster
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_node_memory_total_mb'
  AND d.timestamp >= toDateTime(1770524118) AND d.timestamp <= toDateTime(1770545718)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodename'] IN ('apmistcs-system-2-infra-bdwxn-2djm2')
GROUP BY time, metric
...
### {{ .Values.companyPrefix }}_node_cpu_used_cores
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_node_cpu_used_cores'
  AND d.timestamp >= toDateTime(1770524118) AND d.timestamp <= toDateTime(1770545718)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodename'] IN ('apmistcs-system-2-infra-bdwxn-2djm2')
GROUP BY time, metric
...
### {{ .Values.companyPrefix }}_node_cpu_total_cores
#### Описание
...
### {{ .Values.companyPrefix }}_node_filesystem_used_mb
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_node_filesystem_used_mb'
  AND d.timestamp >= toDateTime(1770524118) AND d.timestamp <= toDateTime(1770545718)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodename'] IN ('apmistcs-system-2-infra-bdwxn-2djm2')
GROUP BY time, metric
...
### {{ .Values.companyPrefix }}_node_filesystem_total_mb
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_node_filesystem_total_mb'
  AND d.timestamp >= toDateTime(1770524118) AND d.timestamp <= toDateTime(1770545718)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodename'] IN ('apmistcs-system-2-infra-bdwxn-2djm2')
GROUP BY time, metric
...
### {{ .Values.companyPrefix }}_node_filesystem_used_percent
#### Описание
Потребление дискового пространства на ноде
#### Лейблы
- cluster
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_node_filesystem_used_percent'
  AND d.timestamp >= toDateTime(1770524118) AND d.timestamp <= toDateTime(1770545718)
  AND t.tags['cluster']  IN ('dlputim6-apmistcs-system-2-infra')
  AND t.tags['nodename'] IN ('apmistcs-system-2-infra-bdwxn-2djm2')
GROUP BY time, metric
...
## Метрики Pod'ов
Метрики и примеры графиков можно посмотреть в графане на дашборде {{ .Values.companyPrefix }}-grafana -> ClickhouseTSDB / Pods (по пути https://GRAFANA_ADDR/grafana/d/clickhouse-tsdb-pods)


### {{ .Values.companyPrefix }}_pod_status_phase
#### Описание
...
WITH last AS (
  ...
  INNER JOIN timeSeriesTags(metrics.ts) AS t
    ON t.id = d.id
  WHERE
    t.metric_name = '{{ .Values.companyPrefix }}_pod_status_phase'
...
### {{ .Values.companyPrefix }}_pod_restarts_total
#### Описание
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_pod_restarts_total'
...
### {{ .Values.companyPrefix }}_pod_info
#### Описание
Информация о Pod'е
#### Лейблы
- cluster
...
INNER JOIN timeSeriesTags(metrics.ts) AS t
  ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_pod_info'
...
### {{ .Values.companyPrefix }}_pod_memory_usage_mb
#### Описание
...
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_pod_memory_usage_mb'
...
### {{ .Values.companyPrefix }}_pod_cpu_usage_cores
#### Описание
...
JOIN timeSeriesTags(metrics.ts) t ON t.id = d.id
WHERE
  t.metric_name = '{{ .Values.companyPrefix }}_pod_cpu_usage_cores'
